...
fit.copy.number = function(samplename, outputfile.prefix, inputfile.baf.segmented, inputfile.baf, inputfile.logr, dist_choice, ascat_dist_choice, min.ploidy=1.6, max.ploidy=4.8, min.rho=0.1,  max.rho=1.0, min.goodness=63, uninformative_BAF_threshold=0.51, gamma_param=1, use_preset_rho_psi=F, preset_rho=NA, preset_psi=NA, read_depth=30, analysis="paired", PURPLE_purity_path, Tx421_WES_purity_path) {
  ...
  if(use_preset_rho_psi){
    ...
    distance.outfile=paste(outputfile.prefix, "runASCAT_distance.png", sep="", collapse="") # kjd 20-2-2014 KT: changing the name to make it more intuitive, including "runASCAT" in name
  ...
  for(i in 1:num_alt_sol){
    
    print(paste0("solution ", i))
    
    ascat_optimum_pair_sol <- list(psi = all_alternative_solutions[i, "psi"], 
                                   rho = all_alternative_solutions[i, "rho"],
                                   solution_type = all_alternative_solutions[i, "solution_type"])

    #KT: make names for plots 
    plot_name <- paste0(ascat_optimum_pair_sol$solution_type, "_psi", ascat_optimum_pair_sol$psi, "_", "rho", ascat_optimum_pair_sol$rho, "_")
    distance.outfile=paste(outputfile.prefix, plot_name, "runclonalASCAT_distance.png",sep="",collapse="") # kjd 20-2-2014 KT: changing the name from "second" to "runclonalASCAT"
    copynumberprofile.outfile=paste(outputfile.prefix, plot_name, "runclonalASCAT_copynumberprofile.png",sep="",collapse="") # kjd 20-2-2014
  ...
  # KT: save a small file with all the solutions, so we can read that back in for callSubclones()
  all_solutions_rho_psi <- all_alternative_solutions
  all_solutions_rho_psi$solution <- c(1:nrow(all_solutions_rho_psi))
  
  write.table(all_solutions_rho_psi,paste(outputfile.prefix, "all_solutions_rho_psi.txt",sep=""),quote=F,sep="\t")
...
#' @author sd11
#' @export
make_posthoc_plots = function(samplename, logr_file, subclones_file, rho_psi_file, bafsegmented_file, logrsegmented_file, allelecounts_file=NULL, RHO, PSI, solution_type) {
  ...
  totalcn_chrom_plot(samplename, subclones, logr, paste0(samplename, "_", solution_type, "_psi", PSI, "_rho", RHO, "_totalcn_chrom_plot.png"), purity)
  ...
  outputfile = paste0(samplename, "_", solution_type, "_psi", PSI, "_rho", RHO, "_alleleratio.png")
  ...
  if (!is.null(allelecounts_file)) {
    ...
    outputfile = paste0(samplename, "_", solution_type, "_psi", PSI, "_rho", RHO, "_coverage.png")
